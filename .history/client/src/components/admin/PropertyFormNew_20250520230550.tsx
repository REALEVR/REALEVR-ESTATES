...
export default function PropertyForm({ property: initialProperty, onSuccess }: PropertyFormProps) {
  ...
  const [imagePreview, setImagePreview] = useState<string | null>(initialProperty?.imageUrl || null);
  ...
  // Update the form when property changes
  useEffect(() => {
    if (property) {
      form.reset({
        ...property,
...
const onSubmit = async (data: PropertyFormValues) => {
    ...
    if (response.ok) {
      if (!property) {
        ...
        if (newProperty) {
          ...
          // Update the component state with the new property instead of reloading
  ...
  const [tourPreviewUrl, setTourPreviewUrl] = useState<string | null>(initialProperty?.tourUrl || null);

  // Update tourPreviewUrl when property changes
  useEffect(() => {
    if (property?.tourUrl) {
      setTourPreviewUrl(property.tourUrl);
    }
  }, [property]);
  ...
  // or switch to tour tab if we just saved a property
  useEffect(() => {
    if (!property?.id) {
      setTabValue('details');
    }
