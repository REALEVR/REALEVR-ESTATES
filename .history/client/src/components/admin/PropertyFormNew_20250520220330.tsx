...
const onSubmit = async (data: PropertyFormValues) => {
  ...
  const propertyData = {
    ...
    squareFeet: Number(data.squareFeet), // Map squareFeet to squareMeters for backend
  ...
  try {
    ...
    if (property) {
      ...
      response = await apiRequest('POST', '/api/properties/createcreate_propertypertyData);
    }
    if (response.ok) {
      if (!property) {
