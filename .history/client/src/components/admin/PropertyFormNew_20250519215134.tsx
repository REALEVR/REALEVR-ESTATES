  ...
  const onSubmit = async (data: PropertyFormValues) => {
    try {
      ...
      if (response.ok) {
        ...
        if (!property) {
          // If creating, get the new property from response
        ...
        if (!property) {
          // Don't close the popup, instead switch to tour tab
          setTabValue('tour');
          setLocalStorageItem('propertyFormTab', 'tour');
          // Optionally, you can update the form with the new property id if needed
          // form.reset({ ...defaultValues, ...newProperty });
          // setImagePreview(newProperty.imageUrl || null);
        }
        // Only call onSuccess if editing, not creating
