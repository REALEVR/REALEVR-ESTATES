import {
  ...
  type PropertyType, type InsertPropertyType
} from "@shared/schema";
import { 
  TABLES, 
  DynamoDBUtils, 
...
export class DynamoDBStorage implements IStorage {
  ...
  async getPropertyViewAnalytics(propertyId: number): Promise<{
    ...
    return executeWithRetry(async () => {
      ...
      const views = await DynamoDBUtils.query(
        TABLES.PROPERTY_VIEWS,
  ...
  async getAgentAnalytics(agentId: number): Promise<{
    ...
    return executeWithRetry(async () => {
      ...
      const topPerformingProperty = properties.reduce((top, prop) => 
        (prop.viewCount || 0) > (top?.viewCount || 0) ? prop : top, null as Property | null
      );
      ...
      const propertiesByCategory = Array.from(categoryCounts.entries())
    ...
    return executeWithRetry(async () => {
      ...
      properties.forEach(prop => {
        if (prop.ownerId) {
          ...
          if (agent) {
            const current = agentStats.get(prop.ownerId) || { propertyCount: 0, totalViews: 0, agentName: agent.fullName };
      ...
      const recentActivity = [
        { type: 'property_added', description: 'New property added by agent', timestamp: new Date().toISOString() },
        { type: 'view_tracked', description: 'Property view recorded', timestamp: new Date(Date.now() - 3600000).toISOString() },
        { type: 'user_registered', description: 'New user registered', timestamp: new Date(Date.now() - 7200000).toISOString() }
      ];
  ...
  async getPropertiesByOwner(ownerId: number): Promise<Property[]> {
    return executeWithRetry(async () => {
      ...
      console.log("All properties:", items.map(item => ({ id: item.id, ownerId: item.ownerId, title: item.title })));
      
      // Filter properties by ownerId
      const filteredItems = items.filter(item => {
        console.log("Checking property:", item.id, "ownerId:", item.ownerId, "ownerId type:", typeof item.ownerId);
      ...
      console.log("Filtered properties:", filteredItems.map(item => ({ id: item.id, ownerId: item.ownerId, title: item.title })));
  ...
  async getAllTourPayments(): Promise<Array<{
    id: number;
    transactionId: string;
    propertyId: number;
  ...
  async getAgentSubscriptions(): Promise<Array<{
    agentId: number;
    agentName: string;
    email: string;
    companyName?: string;
  ...
  async getAgentPropertiesForAdmin(agentId: number): Promise<Array<{
    propertyId: number;
    title: string;
    location: string;
    price: number;
