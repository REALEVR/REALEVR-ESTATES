...
import {
  ...
  //   the new creatd property images dont  show
  Edit,
  Plus,
  Trash2,
...
export default function PropertyManager() {
  ...
  const filteredProperties = properties?.filter(property => {
    // Only include real properties (must have title, location, and price)
    if (!property.title || !property.location || property.price == null) return false;

    // Filter by search query
    const matchesSearch = !searchQuery ||
      property.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
      property.location.toLowerCase().includes(searchQuery.toLowerCase()) ||
      (property.description ? property.description.toLowerCase().includes(searchQuery.toLowerCase()) : false);
  ...
  // Computed state for selection
  const isAllSelected = filteredProperties && filteredProperties.length > 0 &&
    filteredProperties.every(property => selectedProperties.has(property.id));
  const isIndeterminate = filteredProperties && filteredProperties.length > 0 &&
  ...
  const handleSelectAll = () => {
    if (isAllSelected) {
      ...
      const allIds = new Set(filteredProperties?.map(property => property.id) || []);
  ...
  const handleBatchDelete = async () => {
    ...
    const confirmMessage = `Are you sure you want to delete ${selectedProperties.size} selected ${selectedProperties.size === 1 ? 'property' : 'properties'}? This action cannot be undone.`;
    ...
    try {
      const deletePromises = Array.from(selectedProperties).map(async (propertyId) => {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(`Failed to delete property ${propertyId}: ${errorData.message || 'Unknown error'}`);
        }
        return propertyId;
      });
      ...
      toast({
        ...
        description: `Successfully deleted ${selectedProperties.size} ${selectedProperties.size === 1 ? 'property' : 'properties'}`,
      });

      // Clear selection and refresh the properties list
      setSelectedProperties(new Set());
      queryClient.invalidateQueries({ queryKey: ['/api/properties'] });
    } catch (error: any) {
      toast({
        title: "Error",
        description: error.message || "Failed to delete some properties",
        variant: "destructive",
      });
    }
  };
