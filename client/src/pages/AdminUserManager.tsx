...
export default function AdminUserManager() {
  ...
  const getPlanBadge = (plan: string) => {
    ...
    return <Badge variant={variants[plan as keyof typeof variants] || "secondary"}>
              ...
              <div className="space-y-4">
                {agentSubscriptions.length === 0 ? (
                  ...
                  agentSubscriptions.map((agent) => (
                    <div key={agent.agentId} className="border rounded-lg p-6 hover:shadow-md transition-shadow">
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <div className="flex items-center space-x-2">
                          {getSubscriptionStatusBadge(agent.subscriptionStatus, agent.isExpired)}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                        ...
                        <div className="bg-gray-50 rounded-lg p-3">
                          ...
                          <p className="text-xl font-bold">{agent.totalViews.toLocaleString()}</p>
                        </div>
                        <div className="bg-gray-50 rounded-lg p-3">
                          <p className="text-sm text-muted-foreground">Subscription Ends</p>
                          <p className="text-sm font-semibold">{formatDate(agent.membershipEndDate)}</p>
                        </div>
                        <div className="bg-gray-50 rounded-lg p-3">
                          <p className="text-sm text-muted-foreground">Days Left</p>
                          <p className={`text-sm font-semibold ${agent.daysUntilExpiry <= 7 ? 'text-red-600' : 'text-green-600'}`}>
                      ...
                      <div key={agent.agentId} className="flex items-center justify-between p-3 border rounded-lg">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">{agent.propertyCount} properties</p>
                          </div>
                        </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{getPlanBadge(selectedAgent.subscriptionPlan)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Status:</span>
                    <span>{getSubscriptionStatusBadge(selectedAgent.subscriptionStatus, selectedAgent.isExpired)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Started:</span>
                    <span className="font-medium">{formatDate(selectedAgent.membershipStartDate)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Ends:</span>
                    <span className="font-medium">{formatDate(selectedAgent.membershipEndDate)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Days Left:</span>
                    <span className={`font-medium ${selectedAgent.daysUntilExpiry <= 7 ? 'text-red-600' : 'text-green-600'}`}>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="text-center">
                    <p className="text-2xl font-bold text-blue-600">{selectedAgent.propertyCount}</p>
                    <p className="text-sm text-muted-foreground">Total Properties</p>
                  </div>
                  <div className="text-center">
                    <p className="text-2xl font-bold text-green-600">{selectedAgent.totalViews.toLocaleString()}</p>
                    <p className="text-sm text-muted-foreground">Total Views</p>
                  </div>
                  <div className="text-center">
                    <p className="text-2xl font-bold text-purple-600">
                      {selectedAgent.propertyCount > 0 ? Math.round(selectedAgent.totalViews / selectedAgent.propertyCount) : 0}
                    </p>
                ...
                {agentProperties.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {agentProperties.map((property) => (
                      <div key={property.propertyId} className="border rounded-lg p-4 hover:shadow-sm transition-shadow">
                        <div className="flex items-center justify-between">
                          <div>
                          ...
                          <div className="text-right">
                            <p className="text-lg font-bold text-blue-600">{property.viewCount}</p>
                            <p className="text-xs text-muted-foreground">views</p>
