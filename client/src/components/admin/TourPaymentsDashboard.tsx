...
export default function TourPaymentsDashboard() {
  ...
  const { data: tourPayments, isLoading, error } = useQuery<TourPayment[]>({
  ...
  const recentPayments = tourPayments?.filter(payment => {
  ...
  const handleExportData = () => {
    ...
    const csvContent = [
      // CSV Header
      ['Transaction ID', 'Property Title', 'Property Location', 'User Name', 'User Email', 'Amount', 'Currency', 'Payment Date', 'Created Date'].join(','),
      // CSV Data
                ...
                {filteredPayments.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-8 text-gray-500">
                      {searchTerm ? 'No payments found matching your search.' : 'No tour payments yet.'}
                    </TableCell>
                  </TableRow>
                ) : (
